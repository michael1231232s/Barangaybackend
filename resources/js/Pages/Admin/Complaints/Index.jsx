...
export default function Index() {
    ...
    const { q, setQ, status, setStatus, onSubmit } = useIndexFilters(filters, 'admin.complaints.index');
        ...
        <AuthenticatedLayout
            header={
                <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <RefreshListButton title="Refresh complaints" />
                </div>
            }
        >
                    ...
                    <div className="mb-4 rounded-xl border border-blue-900/10 bg-white p-4 shadow-sm">
                        <form onSubmit={onSubmit} className="flex flex-col gap-3 md:flex-row md:items-center">
                            <input
                                ...
                                placeholder="Search resident, category, subject, description, location"
                            />
                            <select
                                value={status}
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-slate-200">
                                <thead className="bg-slate-50">
                                    <tr>
                                        <th className="px-4 py-3 text-left text-xs font-extrabold uppercase tracking-wider text-slate-600">Resident</th>
                                        <th className="px-4 py-3 text-left text-xs font-extrabold uppercase tracking-wider text-slate-600">Category</th>
                                        <th className="px-4 py-3 text-left text-xs font-extrabold uppercase tracking-wider text-slate-600">Subject</th>
                                        <th className="px-4 py-3 text-left text-xs font-extrabold uppercase tracking-wider text-slate-600">Status</th>
                                        <th className="px-4 py-3 text-left text-xs font-extrabold uppercase tracking-wider text-slate-600">Submitted</th>
                                        <th className="px-4 py-3 text-left text-xs font-extrabold uppercase tracking-wider text-slate-600">Actions</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-200 bg-white">
                                    {items.length === 0 ? (
                                        <tr>
                                            <td className="px-4 py-4 text-sm text-gray-500" colSpan={6}>No complaints found.</td>
                                        </tr>
                                    ) : (
                                        items.map((c) => (
...
function ComplaintRow({ row }) {
    ...
    } = useUpdateRowForm(row.id, 'admin.complaints.update', {
        status: row.status,
    ...
    return (
        <>
            <ConfirmModal
                ...
                title="Save changes"
                message="Save updates to this complaint?"
                confirmText="Save"
            ...
            <tr>
                <td className="px-4 py-3 text-sm font-semibold text-gray-900">
                    <div>{row.user?.name || 'Resident'}</div>
                ...
                <td className="px-4 py-3 text-sm">
                    <span className={statusBadgeClass(row.status)}>{String(row.status || '').toUpperCase()}</span>
                </td>
                    ...
                    <td colSpan={6} className="bg-slate-50 px-4 py-4">
                        <div className="grid gap-3 md:grid-cols-2">
                            <div className="rounded border bg-white p-3">
                                ...
                                <div className="mt-2 whitespace-pre-wrap text-sm text-gray-800">{row.description}</div>
                                {row.location ? (
                                    <div className="mt-2 text-sm text-gray-700"><span className="font-semibold">Location:</span> {row.location}</div>
                                ) : null}
                                {row.photo_url ? (
                                    <div className="mt-2 text-sm text-gray-700"><span className="font-semibold">Photo:</span> {row.photo_url}</div>
                                ) : null}
                            </div>
                            {canUpdate ? (
                                <div className="rounded border bg-white p-3">
                                    <form onSubmit={submit} className="flex flex-col gap-3">
                                        ...
                                        <div>
                                            <div className="text-xs font-semibold uppercase tracking-wider text-gray-500">Admin notes</div>
                                            <textarea
                                                value={data.admin_notes}
                                                onChange={(e) => setData('admin_notes', e.target.value)}
                                                rows={4}
                                                className="mt-2 w-full rounded-md border-slate-300 shadow-sm focus:border-blue-600 focus:ring-blue-600"
                                                placeholder="Optional notes for resolution/action"
                                            />
                                        </div>
                                        <button
                                            type="submit"
